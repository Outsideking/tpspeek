#!/bin/bash
# TPspeek Auto Deploy Script on Alibaba Cloud ECS
# Usage: bash deploy_tpspeek.sh your_domain.com your_openai_api_key

DOMAIN=$1
OPENAI_KEY=$2

if [ -z "$DOMAIN" ] || [ -z "$OPENAI_KEY" ]; then
  echo "Usage: bash deploy_tpspeek.sh your_domain.com your_openai_api_key"
  exit 1
fi
